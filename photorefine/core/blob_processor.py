"""
Blob removal processor with history management and multi-pass support
"""
...
class BlobRemovalProcessor:
    """
    Advanced blob removal processor with history management and multi-pass filtering
    """
    ...
    def load_image_array(self, image_array):
        """Load image from numpy array (RGB format)"""
    ...
    def inpaint_regions(self, image, mask, inpaint_radius, method='telea'):
        """Inpaint masked regions using specified method"""
        from ..inpainting import BasicInpainting

        mask_binary = (mask > 0).astype(np.uint8) * 255
        mask_dilated = self.processor.dilate(mask_binary, kernel_size=3, iterations=1)

        result = BasicInpainting.inpaint(image, mask_dilated, method=method, radius=inpaint_radius)
    ...
    def process_with_params(self, params, use_working_image=True, add_to_history=True):
        ...
        if method == 'hsv':
            ...
            mask = WatershedSegmentation.watershed_segmentation(
                image,
                params.get('watershed_markers', 5),
                params.get('watershed_compactness', 0.001)
            )
        else:
            mask = np.zeros((image.shape[0], image.shape[1]), dtype=np.uint8)

        # Apply morphological operations
        if params.get('morph_operation', 'none') != 'none':
            mask = self.morphological_operations(mask, params['morph_operation'],
                                                 params['kernel_size'],
                                                 params.get('morph_iterations', 1))

        # Filter by area
        mask = self.filter_contours_by_area(mask, params.get('min_area', 0), params.get('max_area', 999999))
        ...
        if params.get('enable_inpaint', False):
            ...
            result = self.inpaint_regions(image, mask, params.get('inpaint_radius', 5), method=inpaint_method)
        else:
            result = image
        ...
        if add_to_history and params.get('enable_inpaint', False):
